<header class="article-header">
    <h1>All Systems Go!</h1> Published <time datetime="2023-09-18">2023-09-18</time> by James Frost.
    <p class="tagline">My take aways from the recent user-space Linux conference.</p>
</header>
...
systemd-measure: Generates PCR11 signatures for embedding into a UKI.
ukify: Python script to generate UKIs.
...
Facebook have lots of machines, running lots of services.
Each running service has its own btrfs subvolume. They are generated by btrfs send/receive.

Service only ever writes to its own subvolume. 
Multiple services on a single machine, each with their own subvolume.
...
Confexts are like sysexts but for /etc, and are meant for bringing config under a clear seperation between vendor and user.

If you have certificates in the Machine Ownder Keyring (MOK), you can dm-verity extensions. Purely additive extensions can be applied at runtime atomically.

Can either pin to specific OS version, or statically link everything.

Early boot settings arn't always applied. Software should work without anything in /etc. confext always mounts with noexec, so binaries and scripts should always be put in a sysext.

Originally designed for Azure boost (details in a later talk).

Other usecases where it is useful for people to be able to easily add or remove their own software, or other tools.

What is the usecase over a container image?
...
Started outlining the rrquirements.
...
Want to provide the "Let's Encrypt" of congidential compute, ensuring that everyone can access it.

Confidential Virtual Macines allow lift and shift style move to confidential compute, and depermissions the host.
...
Issues around nested virtulisation, so ideally want to use bare metal servers. This has a startup performance cost.
...
Meta has millions of hosts, mostly homogenious, but increasing hardware diversity.
...
systemd-oomd was originally devvelopped at Meta, its a userspace oom killer that has more flexible policies than the kernel oom killer.
...
frame pointers added into fedora 38. Slightly slows things dure to using an extra register, but makes profiling and debugging much easier. Originally came from someone wanting better traces in systemd.
...
* sched_ext allows experimenting with linux scheduling. Upstreaming work is ongoing.
...
Is there a way to add additional directories to distroboxs? like NFS mounts?

How does systemwide config work? Such as NFS mounts.
/etc is still user editable. Layered mounting for config.
...
Does image based deployment make sense for a desktop usecase?
